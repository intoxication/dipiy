class TopicsController < ApplicationController
  ...
  def action
    ...
    when "excellent"
      @topic.excellent!
      redirect_to @topic, notice: "Plus fine success."
    when "unexcellent"
      @topic.unexcellent!
      redirect_to @topic, notice: "Plus fine has been canceled."
    when "ban"
      params[:reason_text] ||= params[:reason] || ""
      @topic.ban!(reason: params[:reason_text].strip)
      redirect_to @topic, notice: "Has moved to the NoPoint node."
    when "close"
      @topic.close!
      redirect_to @topic, notice: "The topic is closed and will no longer accept any new replies."
    when "open"
      @topic.open!
      redirect_to @topic, notice: "Topic has been restarted."
    end
  end
