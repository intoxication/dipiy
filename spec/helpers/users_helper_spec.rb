...
describe UsersHelper, type: :helper do
  ...
  describe 'user_name_tag' do
    ...
    it 'should result empty with nil param' do
      expect(helper.user_name_tag(nil)).to eq('anonymous')
    end
  end
  ...
  describe '.render_user_level_tag' do
    ...
    it 'admin should work' do
      ...
      is_expected.to eq '<span class="label label-danger role">administrator</span>'
    end

    it 'vip should work' do
      allow(user).to receive(:verified?).and_return(true)
      is_expected.to eq '<span class="label label-success role">Senior member</span>'
    end

    it 'blocked should work' do
      allow(user).to receive(:blocked?).and_return(true)
      is_expected.to eq '<span class="label label-warning role">Prohibit users</span>'
    end

    it 'newbie should work' do
      allow(user).to receive(:newbie?).and_return(true)
      is_expected.to eq '<span class="label label-default role">Novice</span>'
    end

    it 'normal should work' do
      is_expected.to eq '<span class="label label-info role">member</span>'
    end
  end
  ...
  describe '.reward_user_tag' do
    ...
    it 'should workd' do
      ...
      expect(html).to eq %Q(<a class="btn btn-success" data-remote="true" href="/#{user.login}/reward"><i class='fa fa-qrcode'></i> <span>Support support</span></a>)
      html = helper.reward_user_tag(user, class: 'btn btn-default')
      expect(html).to eq %Q(<a class="btn btn-default" data-remote="true" href="/#{user.login}/reward"><i class='fa fa-qrcode'></i> <span>Support support</span></a>)
    end
  end
end
