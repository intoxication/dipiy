...
describe TopicsHelper, type: :helper do
  describe 'topic_favorite_tag' do
    ...
    it 'should result when logined user did not favorite topic' do
      ...
      expect(res).to eq("<a title=\"Collection\" class=\"bookmark \" data-id=\"1\" href=\"#\"><i class=\"fa fa-bookmark\"></i> Collection</a>")
    end

    it 'should result when logined user favorited topic' do
      allow(user).to receive(:favorite_topic?).and_return(true)
      allow(helper).to receive(:current_user).and_return(user)
      expect(helper.topic_favorite_tag(topic)).to eq("<a title=\"Cancel collection\" class=\"bookmark active\" data-id=\"1\" href=\"#\"><i class=\"fa fa-bookmark\"></i> Collection</a>")
    end
  ...
  describe 'topic_follow_tag' do
    ...
    context 'was unfollow' do
      it 'should work' do
        allow(helper).to receive(:current_user).and_return(user)
        expect(helper.topic_follow_tag(topic)).to eq "<a data-id=\"#{topic.id}\" class=\"follow\" href=\"#\"><i class=\"fa fa-eye\"></i> attention</a>"
      end
    end
    ...
    context 'was active' do
      it 'should work' do
        ...
        expect(helper.topic_follow_tag(topic)).to eq "<a data-id=\"#{topic.id}\" class=\"follow active\" href=\"#\"><i class=\"fa fa-eye\"></i> attention</a>"
      end
    end
  end
end
